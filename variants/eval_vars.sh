...
if [[ -z ${alignment} ]]; then
    >&2 echo "Error: Alignment file (-a, --alignment) is not provided!"
    exit 1
elif [[ -z ${golden} ]]; then
...
if [[ ! -f ${regions} ]]; then
    >&2 echo "Error: There is no file ${regions}!"
    exit 1
fi
...
if [[ ! -f ${output}/freebayes.vcf.gz ]]; then
...
if [[ ! -d ${output}/eval-freebayes ]]; then
